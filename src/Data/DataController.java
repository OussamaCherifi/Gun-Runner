...
public class DataController {
    ...
    private static boolean isEasy() {
        String query = "Select easy from difficulty where id = 1";
        try {
            Statement stmt = conn.createStatement();
            ResultSet isEasy = stmt.executeQuery(query);
            if (isEasy.getInt("easy") == 0) {
                return false;
            } else {
                return true;
            }
    ...
    private static boolean isNormal() {
        String query = "Select normal from difficulty where id = 1";
        try {
            Statement stmt = conn.createStatement();
            ResultSet isNormal = stmt.executeQuery(query);
            if (isNormal.getInt("normal") == 0) {
                return false;
            } else {
                return true;
            }
    ...
    private static boolean isHard() {
        String query = "Select hard from difficulty where id = 1";
        try {
            Statement stmt = conn.createStatement();
            ResultSet isHard = stmt.executeQuery(query);
            if (isHard.getInt("hard") == 0) {
                return false;
            } else {
                return true;
            }
    ...
    public static Difficulty getDifficulty() {
        ...
        if(isEasy()) return Difficulty.EASY;
        if(isNormal()) return Difficulty.NORMAL;
        if(isHard()) return Difficulty.HARD;
        
        return Difficulty.EASY;
    }
    
    public static void setDifficultyToEasy(){
        String query = "update difficulty\n"
                + "set easy = 1 \n"
                + "where id = 1";
        String query2 = "update difficulty\n"
                + "set normal = 0 \n"
                + "where id = 1";
        String query3 = "update difficulty\n"
                + "set hard = 0 \n"
                + "where id = 1";
